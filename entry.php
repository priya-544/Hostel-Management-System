...
<form method="POST" action="">
	...
	<span><input type="text" name="RoomNo" autocomplete="off" required></span>
	<br>
    <label style="padding-left:5px">Name:</label >
	<span><input type="text" name="Name" autocomplete="off" required></span>
	<br>
	<label style="padding-left: 10px;">Place:</label>
	<span><select name="Place"> 
	<option 

	<?php
		if (isset($_POST['Place']) && $_POST['Place']=='College') echo "selected='selected'";
	?>

	>College</option>
	<option
	<?php
		if (isset($_POST['Place']) && $_POST['Place']=='Phata') echo "selected='selected'";
	?>
	>
	Phata</option>
</select>
...
<input class="button" type="Submit" name="entry" value="Submit"  >
</form>
</div>
</center>
<?php
$host="localhost";
	...
	{	
		...
		{
		$c_type = filter_input(INPUT_POST ,'Place');
		echo $c_type;
		echo $Name;
		echo $RoomNo;

		$sql ="INSERT INTO entry (Date,RoomNo,Name,Place) values ('$Date','$RoomNo','$Name','$c_type') ";
		if(mysqli_query($con,$sql))
			{
			echo"
			<script >
				alert('Entry registerd!!');
